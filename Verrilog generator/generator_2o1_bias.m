...
bit_frac_out = 10;
% conversion function
[val_list,bin_list] = makelist(bit_out-bit_in);
...
w{1} = [[ 0.4051258 ,  3.6547644 ,  4.469644  , -0.77785814, -0.21705866, -4.6761184 , -4.0280623 ]; ...
       [-0.08454749,  0.7461908 ,  0.578306  , -0.26741946, -0.2889852 , 0.38556305,  0.39222288]];
b{1} = [-0.16548006, -0.41354084, -0.45586804,  0.        ,  0.        , 1.465102  ,  1.0054986 ];
w{2} = [[ 0.1000149 ]; ...
       [-3.5672517 ]; ...
       [-4.0355887 ]; ...
       [ 0.45519632]; ...
...
for i=1:depth
    ...
    fprintf(fID,tab1);
    tempstr = strcat('wire signed [',int2str(bit_in-1),':0] neg1');
    for j=2:n_layers(i)
    ...
    for k=1:n_layers(i+1)
        fprintf(fID,tab1);
        tempstr = strcat('wire signed [',int2str(bit_out-1),...
            ':0] b',int2str(k)," = ");
        % convert weight into fractional point
        bias = round(b{i}(k)*(2^(bit_frac_out)));
        bias = bias+(bias<0)*2^(bit_out);
        tempstr = strcat(tempstr,int2str(bit_out),"'h",...
            dec2hex(bias),';\n');
        nb{i}(k) = bias./(2^(bit_out));
        fprintf(fID,tempstr);
    end
    ...
    for j=1:n_layers(i)
        for k=1:n_layers(i+1)
            ...
            tempstr = strcat(tab1,'wire signed [',...
                int2str(bit_out-1),':0] m',int2str(j),'_',...
                int2str(k),';\n');
            fprintf(fID,tempstr);
            ...
            if m_len(j,k)==0
                ...
                tempstr = strcat(tempstr,str_expandbit(sign1, bit_in,...
                    bit_frac_in+m_rshift(j,k), bit_out, bit_frac_out),';\n');
                fprintf(fID,tempstr);
            %there are 2 value
            else
                ...
                tempstr = strcat(tab1,'wire signed [',...
                    int2str(bit_in+m_len(j,k)-1),':0] tm',int2str(j),'_',...
                    int2str(k),';\n');
                fprintf(fID,tempstr);
                ...
                fprintf(fID,tab1);
                tempstr = strcat("assign tm",int2str(j),'_',int2str(k),'[');
                tempstr = strcat(tempstr,int2str(bit_in+m_len(j,k)-1),...
                    ':',int2str(m_len(j,k)),"] = ");
                tempstr = strcat(tempstr,sign1," + (");
                tempstr = strcat(tempstr,sign2,">>>");
                ...
                tempstr = strcat(str_expandbit(tempstr, bit_in+m_len(j,k),...
                    bit_frac_in+m_rshift(j,k), bit_out, bit_frac_out),';\n');
                fprintf(fID,tempstr);
            end
        end
...
% ouput parameter 
fprintf(fID,tab1);
tempstr = strcat('output [',int2str(bit_out-1),':0] out1');
...
for i=2:depth
    ...
    % activation function
    for j=1:n_layers(i)
        tempstr = strcat(tab1,'actifunc #(',int2str(bit_out),') AF',...
    ...
    % head
    starting = bit_frac_out - bit_frac_in;
    ending = starting + bit_in -1;
...
% ouput parameter 
fprintf(fID,tab1);
tempstr = strcat('wire [',int2str(bit_out-1),':0] out1');
for j=2:n_layers(end)
    tempstr = strcat(tempstr,',out',int2str(j));
end
tempstr = strcat(tempstr,';\n');
...
% testData = 0.2+0.1*rand(1,30);
% testData = [testData;rand(1,30)];
